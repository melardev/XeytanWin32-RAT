...
Thread::Thread(LPTHREAD_START_ROUTINE lpThreadRoutine, LPVOID threadArg, DWORD creationFlags, DWORD sSize,
               BOOL inheritable)
{
	sa = static_cast<LPSECURITY_ATTRIBUTES>(HeapAlloc(GetProcessHeap(), 0, sizeof(SECURITY_ATTRIBUTES)));
...
Thread::Thread(PTHREAD_START_ROUTINE threadRoutine, LPVOID threadArg)
